  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[700px] h-[400px] bg-blue-100/50 rounded-full blur-[130px]" />
      </div>

      <div className="relative max-w-6xl mx-auto px-4 pt-28 pb-16">
        <div className="text-center mb-14">
          <div className="flex flex-col sm:flex-row justify-center items-center gap-4 mb-6">
            <div className="inline-flex items-center gap-2 bg-blue-50 border border-blue-100 text-blue-600 text-xs font-bold tracking-widest uppercase px-4 py-2 rounded-full">
              Agenda
            </div>
            <Link href="/minhas-inscricoes" className="inline-flex items-center gap-2 bg-white hover:bg-gray-50 text-gray-700 text-xs font-semibold px-4 py-2 rounded-full transition-all border border-gray-200 shadow-sm">
        ...
        {(!events || events.length === 0) ? (
          <div className="flex flex-col items-center justify-center py-20 bg-white border border-gray-200 border-dashed rounded-3xl shadow-sm">
            ...
            <p className="text-gray-700 font-bold mb-2">Nenhum evento programado</p>
            <p className="text-gray-500 text-sm">Os próximos eventos aparecerão aqui assim que forem publicados.</p>
          </div>
        ) : (
            ...
            {events.map(event => {
              ...
              return (
                <Link
                  ...
                  className="group relative bg-white border border-gray-200 rounded-3xl overflow-hidden hover:border-blue-300 hover:shadow-md hover:-translate-y-1.5 transition-all duration-300 ease-out"
                >
                  <div className="absolute inset-0 rounded-3xl bg-blue-50/0 group-hover:bg-blue-50/50 transition-colors duration-300 pointer-events-none" />
                  ...
                  <div className="relative h-44 bg-gray-100 overflow-hidden border-b border-gray-100">
                    ...
                    {date && (
                      <div className="absolute bottom-3 left-3 flex gap-2">
                        <div className="bg-white/95 backdrop-blur-md border border-gray-200 rounded-xl px-3 py-2 text-center shadow-sm">
                          <p className="text-lg font-black text-gray-900 leading-none">{date.getDate().toString().padStart(2, '0')}</p>
                          <p className="text-xs font-bold text-blue-600 uppercase">{MONTHS_SHORT[date.getMonth()]}</p>
                        </div>
                        {event.isCancelled && (
                    ...
                    <div className="absolute top-3 right-3 flex flex-col gap-1.5 items-end">
                      <span className={`text-[10px] font-bold px-2.5 py-1 rounded-lg border shadow-sm bg-white ${
                        event.type === 'culto'
                      ...
                      {isPaid && (
                        <span className="text-[10px] font-bold px-2.5 py-1 rounded-lg border shadow-sm bg-white border-amber-200 text-amber-700">
                          R$ {Number(event.ticket_price).toFixed(2)}
                        </span>
                      )}
                      {isRegistered && (
                        <span className="text-[10px] font-bold px-2.5 py-1 rounded-lg border shadow-sm bg-white border-emerald-200 text-emerald-700 flex items-center gap-1">
                  ...
                  <div className="p-5">
                    ...
                    <div className="space-y-1.5 mb-4">
                      {date && (
                        <div className={`flex items-center gap-2 text-xs ${event.isCancelled ? 'text-red-500 line-through' : 'text-gray-500'}`}>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-blue-600 group-hover:text-blue-700 flex items-center gap-1 text-xs font-bold transition-colors">
                        Ver mais
