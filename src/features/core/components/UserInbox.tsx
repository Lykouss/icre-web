  ...
  return (
    <DropdownMenu.Root open={open} onOpenChange={handleOpenChange} modal={false}>
      <DropdownMenu.Trigger asChild>
        <button className="relative flex items-center justify-center w-10 h-10 rounded-full transition-all focus:outline-none hover:bg-white/5 data-[state=open]:bg-white/10 text-slate-400 hover:text-slate-200">
      ...
      <DropdownMenu.Portal>
        <DropdownMenu.Content
          align="end"
          sideOffset={16}
          className="z-50 w-80 max-h-[28rem] rounded-2xl shadow-2xl overflow-y-auto overflow-x-hidden portal-scroll border animate-in fade-in-0 zoom-in-95 data-[side=bottom]:slide-in-from-top-2"
          style={{ 
        ...
        >
          <div className="sticky top-0 px-4 py-3 border-b border-white/5 bg-slate-900/90 backdrop-blur-md z-10 flex justify-between items-center">
            <h3 className="text-sm font-bold text-white">Caixa de Entrada</h3>
            ...
            {items.length === 0 ? (
              ...
              items.slice(0, 5).map((item) => (
                <DropdownMenu.Item
                  ...
                  className={`relative p-4 outline-none cursor-pointer transition-colors border-b border-white/5 last:border-b-0 hover:bg-white/5 ${!item.is_read ? 'bg-blue-500/5' : ''}`}
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className={`text-sm ${!item.is_read ? 'font-bold text-white' : 'font-medium text-slate-300'}`}>{item.communications.title}</p>
                      <p className="text-xs text-slate-400 mt-1 line-clamp-3 leading-relaxed">{item.communications.message}</p>
                    </div>
                  </div>
                </DropdownMenu.Item>
              ))
