...
export function AboutSection({ content }: { content: AboutContent }) {
  ...
  return (
    <section id="sobre" className="relative py-24 px-6 bg-white dark:bg-slate-900 overflow-hidden transition-colors duration-300" data-theme="light">
      ...
      <div className="relative max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        ...
        >
          <div className="relative rounded-2xl overflow-hidden aspect-4/3 bg-gray-100 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 shadow-sm transition-colors">
            {content.image_url ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={content.image_url} alt="Sobre a ICRE" className="w-full h-full object-cover" />
            ) : (
              <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-gray-100 dark:bg-slate-800 transition-colors">
                ...
                <p className="text-sm text-gray-400 dark:text-slate-500">Imagem configurável no painel</p>
              </div>
            )}
          </div>

          {/* Chip — fundada em */}
          <div className="mt-4 inline-flex items-center gap-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-900/30 rounded-xl px-4 py-3 transition-colors">
            ...
            <div>
              <p className="text-[10px] text-gray-400 dark:text-slate-500 font-semibold uppercase tracking-widest">Fundada em</p>
              <p className="text-xl font-black text-gray-900 dark:text-white leading-tight transition-colors">1993</p>
            </div>
            <div className="ml-2 flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-emerald-400" />
              <span className="text-xs font-semibold text-gray-600 dark:text-slate-300 transition-colors">Santa Maria, Brasília-DF</span>
            </div>
          </div>
        </div>
        ...
        >
          ...
          <h2 className="text-3xl sm:text-4xl font-black text-gray-900 dark:text-white leading-tight mb-5 transition-colors">
