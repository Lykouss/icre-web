...
// â”€â”€ Seletor de mÃ­dia (galeria) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function MediaPicker({ value, onChange }: MediaPickerProps) {
  ...
  const handleDelete = async (item: SiteMediaItem) => {
    if (!confirm(`Excluir "${item.name}"? Blocos que usam essa imagem ficarÃ£o sem imagem.`)) return;
    ...
    <div className="space-y-2">
      ...
      {value && (
        <div className="relative w-full aspect-video rounded-lg overflow-hidden bg-slate-100 border border-slate-200">
          ...
          <button
            onClick={() => onChange('')}
            className="absolute top-2 right-2 p-1 bg-black/60 hover:bg-black/80 text-white rounded-lg transition-colors"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[80vh] flex flex-col overflow-hidden">
            ...
            <div className="px-5 py-3 border-b border-slate-100 shrink-0">
              ...
              {usedPct > 85 && (
                <p className="text-xs text-red-500 mt-1.5 font-medium">EspaÃ§o quase esgotado. Exclua imagens nÃ£o utilizadas.</p>
              )}
            </div>
              ...
              <label className={`flex items-center justify-center gap-2 py-2.5 border-2 border-dashed rounded-xl cursor-pointer transition-colors ${uploading ? 'border-slate-200 bg-slate-50 text-slate-500 dark:text-slate-400' : 'border-slate-300 hover:border-blue-400 hover:bg-blue-50 hover:text-blue-600 text-slate-500'}`}>
                {uploading ? (
                  ...
                  <>
                    ...
                    <span className="text-xs text-slate-500 dark:text-slate-400">JPG, PNG, WebP, GIF Â· MÃ¡x. 10 MB</span>
                  </>
                )}
                      ...
                      <div className="absolute inset-0 bg-black/0 group-hover:bg-black/50 transition-all flex flex-col justify-between p-2 opacity-0 group-hover:opacity-100">
                        <div className="flex justify-end">
                          <button
                            ...
                            className="p-1 bg-red-600 hover:bg-red-500 text-white rounded-lg transition-colors"
                            title="Excluir"
                          >
                        ...
                        <p className="text-white text-xs font-medium truncate">{formatBytes(item.size_bytes)}</p>
                      </div>
                    </div>
                  ))}
                </div>
...
// â”€â”€ Layout panel â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function LayoutPanel({ layout, onChange }: { layout: BlockLayout; onChange: (l: BlockLayout) => void }) {
  ...
  const seg = (active: boolean) => `flex-1 flex items-center justify-center py-1.5 rounded-md text-xs font-semibold transition-colors ${active ? 'bg-white text-white shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-600'}`;
...
// â”€â”€ Editores por tipo â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

function HeroEditor({ c, set }: { c: Record<string, unknown>; set: (k: string, v: unknown) => void }) {
  return (
    <>
      <Field label="TÃ­tulo principal">
        <input className={inputCls} value={c.title as string ?? ''} onChange={e => set('title', e.target.value)} placeholder="Bem-vindo Ã  ICRE" />
      </Field>
      <Field label="SubtÃ­tulo">
        <textarea className={inputCls} rows={2} value={c.subtitle as string ?? ''} onChange={e => set('subtitle', e.target.value)} placeholder="Uma breve descriÃ§Ã£o..." />
      </Field>
...
function AboutEditor({ c, set }: { c: Record<string, unknown>; set: (k: string, v: unknown) => void }) {
  return (
    <>
      <Field label="TÃ­tulo">
        <input className={inputCls} value={c.title as string ?? ''} onChange={e => set('title', e.target.value)} placeholder="Sobre nÃ³s" />
      </Field>
      <Field label="Texto">
        <textarea className={inputCls} rows={6} value={c.text as string ?? c.body as string ?? ''} onChange={e => set('text', e.target.value)} placeholder="Um pouco sobre a histÃ³ria e missÃ£o da igreja..." />
      </Field>
      <Field label="Imagem">
        <MediaPicker value={c.image_url as string ?? ''} onChange={v => set('image_url', v)} />
      </Field>
...
function MissionEditor({
  ...
  return (
    <>
      <Field label="TÃ­tulo da seÃ§Ã£o">
        <input className={inputCls} value={c.title as string ?? ''} onChange={e => set('title', e.target.value)} placeholder="Nossa MissÃ£o" />
      </Field>
      ...
      <div className="space-y-3">
        {items.map((item, i) => (
          <div key={i} className="bg-slate-50 border border-slate-200 rounded-xl p-3 space-y-2">
            ...
            <input className={inputCls} placeholder="TÃ­tulo" value={item.title ?? ''} onChange={e => setItem(i, 'title', e.target.value)} />
            <textarea className={inputCls} placeholder="DescriÃ§Ã£o" rows={2} value={item.text ?? ''} onChange={e => setItem(i, 'text', e.target.value)} />
            <input className={inputCls} placeholder="Ãcone (emoji ou sÃ­mbolo)" value={item.icon ?? ''} onChange={e => setItem(i, 'icon', e.target.value)} />
          </div>
        ))}
        <button
          onClick={addItem}
...
function EventsEditor({ c, set }: { c: Record<string, unknown>; set: (k: string, v: unknown) => void }) {
  return (
    <>
      <Field label="TÃ­tulo"><input className={inputCls} value={c.title as string ?? ''} onChange={e => set('title', e.target.value)} placeholder="PrÃ³ximos Eventos" /></Field>
      <Field label="SubtÃ­tulo"><input className={inputCls} value={c.subtitle as string ?? ''} onChange={e => set('subtitle', e.target.value)} placeholder="Venha participar..." /></Field>
      <Hint>Os eventos sÃ£o buscados automaticamente com <strong>is_public = true</strong> e status publicado.</Hint>
    </>
  );
}
...
function YoutubeEditor({ c, set }: { c: Record<string, unknown>; set: (k: string, v: unknown) => void }) {
  return (
    <>
      ...
      <Hint>Cole apenas o ID apÃ³s <code className="bg-slate-100 px-1 rounded">?v=</code> na URL do YouTube.</Hint>
    </>
  );
}
...
function ContactEditor({ c, set }: { c: Record<string, unknown>; set: (k: string, v: unknown) => void }) {
  return (
    <>
      ...
      <Field label="EndereÃ§o"><input className={inputCls} value={c.address as string ?? ''} onChange={e => set('address', e.target.value)} placeholder="Rua, nÃºmero, bairro" /></Field>
      <div className="grid grid-cols-2 gap-3">
      ...
      <Field label="HorÃ¡rio"><input className={inputCls} value={c.schedule as string ?? ''} onChange={e => set('schedule', e.target.value)} placeholder="Dom 9h e 19h / Qua 19h30" /></Field>
...
function CustomTextEditor({ c, set }: { c: Record<string, unknown>; set: (k: string, v: unknown) => void }) {
  return (
    <>
      <Field label="TÃ­tulo"><input className={inputCls} value={c.title as string ?? ''} onChange={e => set('title', e.target.value)} placeholder="TÃ­tulo opcional" /></Field>
...
function BannerEditor({ c, set }: { c: Record<string, unknown>; set: (k: string, v: unknown) => void }) {
  return (
    <>
      ...
      <Field label="Texto alternativo"><input className={inputCls} value={c.alt as string ?? ''} onChange={e => set('alt', e.target.value)} placeholder="DescriÃ§Ã£o da imagem" /></Field>
    </>
  );
}
...
export function BlockEditor({ block, onClose, onContentChange }: BlockEditorProps) {
  ...
  // pendingNotify guarda o conteÃºdo mais recente para notificar fora do updater
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 shrink-0">
        <div>
          ...
          <h3 className="font-bold text-white text-sm">{BLOCK_LABELS[block.type] ?? block.type}</h3>
        </div>
