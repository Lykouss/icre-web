...
function EventModal({ event, onClose }: { event: PublicEvent; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4" onClick={onClose}>
      ...
      <div
        className="relative bg-white border border-gray-200 rounded-t-3xl sm:rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-modal-in"
      ...
      >
        ...
        <div className="p-6 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            {event.time && (
              <div className="flex items-center gap-3 bg-gray-50 border border-gray-200 rounded-xl p-3.5">
                ...
                <div>
                  <p className="text-[10px] text-gray-400 font-bold uppercase tracking-wide">Horário</p>
                  <p className="text-gray-800 font-semibold text-sm">{event.time}</p>
                </div>
              </div>
            )}
            {event.location && (
              <div className="flex items-center gap-3 bg-gray-50 border border-gray-200 rounded-xl p-3.5">
                ...
                <div className="min-w-0">
                  <p className="text-[10px] text-gray-400 font-bold uppercase tracking-wide">Local</p>
                  <p className="text-gray-800 font-semibold text-sm truncate">{event.location}</p>
                </div>
              </div>
            )}
          </div>
          ...
          {event.description && (
            <p className="text-gray-500 text-sm leading-relaxed px-1">{event.description}</p>
          )}
      ...
      >
        ...
        <div className="relative p-7 pt-40 flex flex-col gap-2">
          <div className="flex flex-wrap justify-between w-full items-start gap-2">
            ...
            {event.isCancelled && (
              <span className="flex items-center gap-1.5 text-xs font-black uppercase tracking-widest text-red-700 bg-red-100 border border-red-200 px-3 py-1.5 rounded-full">
                Cancelado
              </span>
            )}
          </div>
  ...
  return (
    <>
      <div
        ref={ref}
        onClick={() => setOpen(true)}
        className={`group relative flex overflow-hidden rounded-xl cursor-pointer transition-all duration-200 ease-out bg-white border border-gray-200 hover:border-blue-300 hover:shadow-md ${event.isCancelled ? 'opacity-60' : ''}`}
        style={{ opacity: visible ? 1 : 0, transform: visible ? 'translateY(0)' : 'translateY(16px)', transitionDelay: `${index * 50}ms` }}
      >
        {/* Barra de data */}
        <div className="relative shrink-0 w-20 sm:w-24 bg-gray-50 border-r border-gray-200 flex flex-col items-center justify-center overflow-hidden">
          {event.banner_url ? (
            <>
            ...
            <div className="flex flex-col items-center justify-center gap-1 w-full h-full py-4">
              {date ? (
                <>
                  <span className="text-2xl font-black text-gray-800 leading-none">{date.day}</span>
                  <span className="text-xs font-bold text-blue-600 uppercase tracking-wider">{date.month}</span>
                </>
              ) : (
        ...
        <div className="flex-1 flex items-center gap-4 px-5 py-4 min-w-0">
          <div className="flex-1 min-w-0">
            {date && <p className="text-[11px] text-gray-400 font-semibold uppercase tracking-wider mb-1">{date.weekday}</p>}
            <h3 className="font-bold text-gray-900 group-hover:text-blue-600 transition-colors duration-200 truncate text-base leading-tight">{event.title}</h3>
            <div className="flex items-center gap-3 mt-1.5 flex-wrap">
              {event.isCancelled && (
                <span className="flex items-center gap-1 text-[10px] uppercase tracking-wider font-bold text-red-600 bg-red-50 border border-red-200 px-2 py-0.5 rounded-md">Cancelado</span>
              )}
              {event.time && (
          ...
          <svg className="w-4 h-4 text-gray-300 group-hover:text-blue-500 group-hover:translate-x-1 transition-all duration-200 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export function EventsSection({ content, events }: Props) {
  ...
  return (
    <section id="eventos" className="relative py-24 px-6 bg-white overflow-hidden" data-theme="light">
      <div className="relative max-w-3xl mx-auto">
        ...
        >
          ...
          <p className="mt-3 text-sm text-gray-400">Clique em um evento para ver mais detalhes</p>
        </div>
        ...
        {events.length > 0 ? (
          <>
            ...
            {specialEvents.length > 0 && regularEvents.length > 0 && (
              <div className="flex items-center gap-4 mb-6">
                ...
                <span className="text-xs text-gray-400 font-semibold uppercase tracking-widest">Cultos regulares</span>
                <div className="flex-1 h-px bg-gray-200" />
              </div>
            )}
            ...
            >
              <Link
                href="/agenda"
                className="inline-flex items-center gap-2 text-gray-500 hover:text-gray-900 text-sm font-semibold border border-gray-300 hover:border-gray-400 px-6 py-3 rounded-xl hover:bg-gray-50 transition-all duration-200"
              >
                Ver agenda completa
          ...
          <div className="bg-gray-50 border border-dashed border-gray-300 rounded-2xl p-14 text-center">
            ...
            <p className="text-gray-600 font-semibold mb-1">Nenhum evento programado</p>
            <p className="text-gray-400 text-sm mb-6">Os próximos eventos aparecerão aqui assim que forem publicados no painel.</p>
            <Link
              href="/agenda"
              className="inline-flex items-center gap-2 text-blue-600 hover:text-blue-700 text-sm font-semibold border border-blue-200 hover:border-blue-300 px-5 py-2.5 rounded-xl hover:bg-blue-50 transition-all duration-200"
            >
              Ver agenda completa
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7"/></svg>
            </Link>
