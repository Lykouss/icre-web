...
function AlertBanner({ events }: { events: PublicEvent[] }) {
  ...
  if (specialSoon) {
    return (
      <div className="mb-8 flex items-center gap-3 bg-amber-500/10 border border-amber-500/25 rounded-2xl px-5 py-4 animate-glow-amber">
        <div className="shrink-0 w-8 h-8 bg-amber-500/15 border border-amber-500/30 rounded-xl flex items-center justify-center">
          <svg className="w-4 h-4 text-amber-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"/>
          </svg>
        </div>
        <div className="flex-1">
          <p className="text-amber-300 font-bold text-sm">Evento Especial em destaque</p>
          <p className="text-amber-400/70 text-xs mt-0.5">{specialSoon.title}</p>
        </div>
