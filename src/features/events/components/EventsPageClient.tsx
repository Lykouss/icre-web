...
export function EventsPageClient({ initialEvents, canManage, isSysAdmin: _isSysAdmin }: EventsPageClientProps) {
    ...
    return (
      <div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-2 flex-wrap">
                    <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold ${
                      selectedEvent.type === 'culto' ? 'bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300' : 'bg-purple-100 text-purple-700'
                    }`}>
                ...
                {canManage && (
                  <div className="flex flex-col gap-2 shrink-0">
                    <button
                      ...
                      className="flex items-center gap-2 px-4 py-2.5 bg-white dark:bg-slate-800 border border-slate-200 hover:border-slate-300 hover:bg-slate-50 text-slate-700 text-sm font-semibold rounded-xl transition-all shadow-sm"
                    >
                      <svg className="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    {_isSysAdmin && (
                      <button
                        ...
                        disabled={isPending}
                        className="flex items-center gap-2 px-4 py-2 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 hover:bg-red-100 text-sm font-semibold rounded-xl transition-all disabled:opacity-50"
                      >
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <Sheet open={isFormOpen} onOpenChange={setIsFormOpen}>
          <SheetContent side="right" className="w-full sm:max-w-2xl overflow-y-auto bg-white dark:bg-slate-800 border-l border-slate-100 p-6 sm:p-8">
  ...
  return (
    <div className="space-y-6">
      {/* Toolbar */}
      <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col sm:flex-row items-start sm:items-center gap-3">
      ...
      {filtered.length > 0 ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 bg-slate-50 rounded-2xl border border-slate-200 border-dashed">
          <div className="w-14 h-14 bg-white dark:bg-slate-800 rounded-full flex items-center justify-center border border-slate-200 shadow-sm mb-4">
