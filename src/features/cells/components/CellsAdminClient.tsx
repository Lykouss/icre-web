...
const MEETING_TYPES: { value: MeetingType; label: string; color: string }[] = [
  { value: 'presencial', label: 'Presencial', color: 'bg-emerald-100 text-emerald-700 dark:text-emerald-300 border-emerald-200 dark:border-emerald-900/40' },
  { value: 'online',     label: 'Online',     color: 'bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 border-blue-200 dark:border-blue-900/40'         },
...
function Toggle({ checked, onChange, disabled }: { checked: boolean; onChange: (v: boolean) => void; disabled?: boolean }) {
  return (
    <button type="button" role="switch" aria-checked={checked} disabled={disabled}
      ...
      <span className={`absolute top-1 left-1 w-4 h-4 bg-white dark:bg-slate-800 rounded-full shadow transition-transform duration-200 ${checked ? 'translate-x-4' : 'translate-x-0'}`} />
    </button>
  );
}
...
function CellModal({ editing, leaders, isPending, onClose, onSubmit }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div className="relative bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select name="leader1_id" defaultValue={editing?.leader1_id ?? ''} className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 bg-white dark:bg-slate-800">
                ...
                <div>
                  ...
                  <select name="leader2_id" defaultValue={editing?.leader2_id ?? ''} className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 bg-white dark:bg-slate-800">
                    <option value="">— Nenhum —</option>
                    {activeLeaders.map(l => <option key={l.id} value={l.id}>{l.name}</option>)}
                  </select>
                </div>
...
function CellDetailModal({ cell, leader1, leader2, onClose, onEdit }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div className="relative bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6 space-y-5">
          <div className="flex items-start justify-between gap-3">
            ...
            <button onClick={() => { onClose(); onEdit(); }} className="shrink-0 flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-blue-600 dark:text-blue-400 border border-blue-200 dark:border-blue-900/40 rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors">
          ...
          <div className="flex flex-wrap gap-2">
            {cell.contact_whatsapp && (
              <a href={`https://wa.me/${cell.contact_whatsapp.replace(/\D/g, '')}`} target="_blank" rel="noreferrer"
                className="flex items-center gap-1.5 text-xs px-3 py-1.5 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-900/40 rounded-lg font-medium hover:bg-emerald-100 transition-colors">
...
function CellCard({ cell, leader1, leader2, canManage, isSysAdmin, onEdit, onDetail, onToggle, toggling, onDelete }: {
  ...
  return (
    <div className={`bg-white dark:bg-slate-800 rounded-2xl border overflow-hidden flex flex-col transition-all duration-200 hover:shadow-md ${cell.is_active ? 'border-slate-200' : 'border-slate-200 opacity-60'}`}>
      {/* Banner */}
      ...
      <div className="p-4 flex flex-col flex-1">
        <button onClick={onDetail} className="text-left mb-2">
          <h3 className="font-bold text-slate-900 text-sm leading-tight hover:text-blue-600 dark:hover:text-blue-400 transition-colors">{cell.name}</h3>
          {cellLeaders.length > 0 && (
        ...
        {canManage && (
          <div className="flex items-center justify-between pt-3 border-t border-slate-100">
            ...
            <div className="flex items-center gap-1">
              <button onClick={onEdit} className="p-1.5 rounded-lg text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors" title="Editar">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
              </button>
              {isSysAdmin && (
                <button onClick={onDelete} className="p-1.5 rounded-lg text-slate-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors" title="Remover">
    ...
    <div className="max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          ...
          { label: 'Ativas',   value: activeCount,    color: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-50 dark:bg-emerald-900/20 border-emerald-200 dark:border-emerald-900/40' },
      ...
      <div className="relative mb-6">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar por nome ou bairro..."
          className="w-full pl-9 pr-4 py-2.5 text-sm border border-slate-200 rounded-xl bg-white dark:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400" />
      </div>
